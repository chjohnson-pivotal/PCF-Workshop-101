...
image::lab.png[]
+
Inside of Cloud Foundry there is a componet called the http://docs.cloudfoundry.org/concepts/architecture/router.html[(Go)Router] that applies a round- robin random routing algorithm to all of the application instances assigned to this route.
As an instance reaches the `running` state, its link:http://docs.pivotal.io/pivotalcf/1-8/concepts/architecture/index.html#diego-cell[Diego Cell] registers that instance in the routing table assigned to its route by sending a message to Cloud Foundry's message bus.
...
The second way is via the Apps Manager UI.  Click on the _Workshop_ application to view application details.  Select the _Route_ tab to view a list of mapped routes:
...
. We can just as easily remove a route by clicking on the red 'x' on the route you wish to remove.
+
image::lab5.png[]
+
