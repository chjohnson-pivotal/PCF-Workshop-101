...
In this labs we'll continue to explore Pivotal Cloudfoundry application operations.
--
...
Application developers can view application logs using the CF CLI, through the App Manager and through PCF Metrics. 
...
. Open up the App Manager by navigating to  https://apps.run.haas-72.pez.pivotal.io/[https://apps.run.haas-72.pez.pivotal.io/] and logging in if necessary
+
image::app-mgr-org-view.png[]
+
. Navigate to the space in which your application is deployed by clicking on the space name on the left
+
image::app-mgr-space-view.png[]
+
. Click on the name of your application (e.g. workshop)
+
image::app-mgr-app-view.png[]
+
. Click on the Logs tab
+
image::applogs-via-app-mgr.png[]

=== Using PCF Metrics

In addition to the tools we have already seen for inspecting logs, there is another way to look at what is going on in your applications. PCF Metrics is designed to help developers better understand the health and performance of the apps running on Pivotal Cloud Foundry.

. Open a new browser tab and navigate to https://metrics.run.haas-90.pez.pivotal.io[https://metrics.run.haas-90.pez.pivotal.io] to open the PCF Metrics page for this PCF instance.
+
image::metrics-start.png[]
+

. In the search box at the top of the screen type in the name of your application.  You should see a dropdown box, and there will likely be multiple apps with the same name.  Choose the app from the drop down that matches the org and space for your student id.
+
image::metrics-dropdown.png[]
+

. Once you have picked your application the page should populate with information about your running applicaiton.  This includes container info, events, and logs.
+
image::metrics-dashboard.png[]
+
The dashboard shows recent events, certain container metrics such as CPU, Memory and Disk and Network Metrics of your application. 
+
. Click on the Explore menu on the left side
+
image::metrics-explore-view.png[]
+
In this view, you get the ability to drill down to what is happening within your application. The time scale at the upper left allows you to either zoom into more granular detail or zoom out to see an overal perspective. You can use the slider on the time line to choose specific timeframes to investigate.
+
In the middle of the page you can use the filter and highlight capabilites to enter words to look for in the application logs shown below. The Sources drop down allows you to include or exclude information specific to your application and/or Cloud Foundry components. Experiment with these capabilities.
...
. If you do not have more than one application instance running, execute the scale command to scale to 2 or more application instances.  
+
```
$ cf scale -i 2 workshop
```
+
...
----
<1> The Diego Cell indicates that it is starting another instance of the application as a result of the Health Manager observing a difference between the desired and actual state (i.e. running instances = 1 vs. running instances = 0).
